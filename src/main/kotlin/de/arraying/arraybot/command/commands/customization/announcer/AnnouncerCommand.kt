 ...
 */
class AnnouncerCommand(override val subCommands: Array<SubCommand>): DefaultCommand("announcer",
    ...
    override fun onCommand(environment: CommandEnvironment, args: List<String>) {
        ...
        val embed = UEmbed.getEmbed(channel)
                .setDescription(Message.COMMANDS_ANNOUNCER_EMBED_DESCRIPTION.getContent(channel))
                .addField(Message.EMBED_TITLE_COMMANDS.getContent(channel),
                        Message.COMMANDS_ANNOUNCER_EMBED_VALUE.getContent(channel),
                        false)
