 ...
 */
class CustomSetValueSubCommand: SubCommand("setvalue",
        aliases = arrayOf("sv", "setmagic")) {
    ...
    override fun onSubCommand(environment: CommandEnvironment, args: List<String>) {
        ...
        if(args.size < 4) {
            Message.COMMANDS_CUSTOM_PROVIDE_VALUE.send(channel).queue()
            return
        }
        ...
        val value = UArguments.combine(args.toTypedArray(), 3)
        if(value.length > Limits.CUSTOM_VALUE.limit) {
            Message.COMMANDS_CUSTOM_DESCRIPTION_LENGTH.send(channel, Limits.CUSTOM_VALUE.limit.toString()).queue()
            return
        }
        val customCommand = Category.CUSTOM_COMMAND.entry as CustomCommandEntry
        customCommand.push(customCommand.getField(CustomCommandEntry.Fields.VALUE), guildId, name, value)
