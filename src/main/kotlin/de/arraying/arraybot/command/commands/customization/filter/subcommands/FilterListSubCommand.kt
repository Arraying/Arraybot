 ...
 */
class FilterListSubCommand: SubCommand("list",
    ...
    override fun onSubCommand(environment: CommandEnvironment, args: List<String>) {
        ...
        val filtered = Category.FILTER.entry as SetEntry
        val phrases = filtered.values(environment.guild.idLong)
        val embed = UEmbed.getEmbed(channel)
                .setDescription(Message.COMMANDS_FILTER_LIST_DESCRIPTION.getContent(channel))
        val pages = PageBuilder()
                .withEntries(phrases.toTypedArray())
                .withTotal(1)
                .withType(PageBuilder.Type.LIST)
                .withTitle(Message.COMMANDS_FILTER_LIST_TITLE.getContent(channel))
                .withEmbed(embed)
                .build()
