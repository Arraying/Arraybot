 ...
 */
class CustomSetTypeSubCommand: SubCommand("settype",
        aliases = arrayOf("st")) {
    ...
    override fun onSubCommand(environment: CommandEnvironment, args: List<String>) {
        ...
        if(args.size < 4) {
            Message.COMMANDS_CUSTOM_PROVIDE_TYPE.send(channel).queue()
            return
        }
        ...
        val type = CustomCommandType.fromString(args[3])
        ...
        customCommand.push(customCommand.getField(CustomCommandEntry.Fields.TYPE), guildId, name, type)
