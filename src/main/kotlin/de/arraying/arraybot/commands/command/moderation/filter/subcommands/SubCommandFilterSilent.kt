...
class SubCommandFilterSilent: 
    ...
    override fun onSubCommand(environment: CommandEnvironment, args: Array<String>) {
        ...
        if(!mod.filterSilent) {
            mod.filterSilent = true
            Messages.COMMAND_FILTER_SILENT_ENABLED.send(channel).queue()
        } else {
            mod.filterSilent = false
            Messages.COMMAND_FILTER_SILENT_DISABLED.send(channel).queue()
        }
    }
