...
class SubCommandAnnouncerJoin:
    ...
    override fun onSubCommand(environment: CommandEnvironment, args: Array<String>) {
        ...
        if(!announcer.joinAnnouncer) {
            announcer.joinAnnouncer = true
            Messages.COMMAND_ANNOUNCER_JOIN_ENABLED.send(channel).queue()
        } else {
            announcer.joinAnnouncer = false
            Messages.COMMAND_ANNOUNCER_JOIN_DISABLED.send(channel).queue()
        }
    }
